...
async def paystack_initialize(req: PaystackInitRequest):
    ...
    if not PAYSTACK_SECRET_KEY:
        raise HTTPException(status_code=500, detail="Paystack not configured. Set PAYSTACK_SECRET_KEY environment variable.")

    # Email is required by Paystack
    if not req.email:
        raise HTTPException(status_code=400, detail="Email address is required to initiate payment. Please sign in first.")
...
# ============================================================
@app.get("/api/leads/task/{task_id}")
async def get_leads_by_task(task_id: str):
    """Get all leads for a specific task by task_id."""
    from supabase_client import get_supabase
    db = get_supabase()
    result = (
