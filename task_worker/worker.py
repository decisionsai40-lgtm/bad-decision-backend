...
async def _deduct_coins(user_id: str, amount: int):
    ...
    try:
        db = get_supabase()
        # Use explicit parameter names to avoid function overloading ambiguity
        # If the RPC fails due to PGRST203 (overloading), fall back to direct update
        try:
            db.rpc("deduct_coins", {"p_user_id": user_id, "p_amount": amount, "p_description": f"Search task charge"}).execute()
        except Exception as rpc_err:
            err_str = str(rpc_err)
            if "PGRST203" in err_str or "overloading" in err_str.lower() or "choose the best candidate" in err_str.lower():
                # Fallback: directly update the coin_balances table
                print(f"[WORKER] deduct_coins RPC overload, using direct update fallback")
                current = db.table("coin_balances").select("balance, total_spent").eq("user_id", user_id).execute()
                if current.data:
                    new_balance = max(0, current.data[0].get("balance", 0) - amount)
                    new_spent = current.data[0].get("total_spent", 0) + amount
