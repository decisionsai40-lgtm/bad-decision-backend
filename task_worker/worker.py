...
async def _process_task(task: Dict[str, Any]):
    ...
    print(f"[WORKER] Processing task {task_id}: {task_type} — '{query}' (tier: {user_tier}, location: {location})")
    ...
    try:
        ...
        # After creating collection, update it with engine_type
        if collection and leads:
            try:
                db = get_supabase()
                db.table("smart_collections").update({
                    "task_type": task_type,
                }).eq("id", collection).execute()
            except:
                pass

        # Step 6: Deduct coins only for leads with real contact data
        valuable_count = _count_valuable_leads(leads)

        if valuable_count == 0:
            # All leads have ABSENT contact data — don't charge
            print(f"[WORKER] No leads with contact data — coins NOT deducted")
            await _update_task_status(task_id, "exhausted")
            print(f"[WORKER] Task {task_id} exhausted — {len(leads)} leads found (0 with contact data)")
            return

        # Deduct coins only for valuable leads
        if coins_reserved > 0 and valuable_count > 0:
            coins_to_deduct = min(coins_reserved, valuable_count)
            await _deduct_coins(user_id, coins_to_deduct)
            print(f"[WORKER] Deducted {coins_to_deduct} coins from user {user_id} ({valuable_count}/{len(leads)} leads with contact data)")
...
def _count_valuable_leads(leads: list) -> int:
    """Count leads that have at least one real contact method."""
    count = 0
    for lead in leads:
        email = lead.get("verified_email", "ABSENT")
        phone = lead.get("phone", "ABSENT")
        linkedin = lead.get("linkedin", "ABSENT")
        if (email != "ABSENT" and email) or (phone != "ABSENT" and phone) or (linkedin != "ABSENT" and linkedin):
            count += 1
    return count
